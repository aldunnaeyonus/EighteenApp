...
import CreditsFont from "../../SubViews/camera/credits";
...
let eventEnd = startDate(props.item.item.end);
  ...
  >
    ...
    >
      ...
      <View style={styles.imageUserNameContainer} pointerEvents= { props.item.item.end >= moment().unix() ? "auto" : "none"}  >
        <Icon
        onPress={() => {
...
<CreditsFont credits={props.item.item.credits} />
        ...
        >
          ...
          props.item.item.autoJoin == "1" && 
            props.item.item.camera_count < parseInt(props.item.item.cameras) + parseInt(props.item.item.camera_cameras_extra) && 
            props.item.item.subscribed == "0" && props.item.item.end >= moment().unix() ?
      ...
      <View
        ...
        >
          ...
          >{i18n.t("Cameras:")}</Text> {props.item.item.camera_count} of{" "}
