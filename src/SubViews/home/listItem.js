...
const ListItem = (props) => {
  ...
  return (
    <SafeAreaProvider key={props.item.item.UUID} style={style.listItem} edges={['right', 'bottom', 'left']} >
    <View
    key={props}
    style={{
      height: 350,
  ...
  >
    ...
    >
      ...
      >
        {props.item.item.title == undefined ? "" : props.item.item.title.toUpperCase()}
      </Text>
      <Text
        numberOfLines={2}
        style={{
    ...
    <View style={styles.imageUserNameContainer}>
        <MenuView
          ...
          title={props.item.item.title == undefined ? "" : props.item.item.title.toUpperCase()}
      ...
      <View
        ...
        >
          ...
          >
            {i18n.t("Cameras:")}</Text> {props.item.item.camera_count} of {parseInt(props.item.item.cameras) + parseInt(props.item.item.camera_cameras_extra)}{" "}{props.item.item.cameras != (props.isPro == "1" ? 1000 : 18) && 
            <TouchableOpacity 
            onPress={()=>{
              props._addMax(
                props.item.item.pin,
          ...
          >{i18n.t("Media:")}</Text> {props.item.item.media_count - 1}
          </Text>
        </View>
         <View
                  style={{
