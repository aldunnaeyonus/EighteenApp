...
const GetPro = (props) => {
  ...
  useFocusEffect(
    ...
    const openSubscriptions = async () => {
        try {
          await deepLinkToSubscriptions(constants.productSkusSubscriptions[0], false); 
        } catch (error) {
...
<SafeAreaProvider>
    ...
    >
        <AnimatedFlatlist
          ...
          ListFooterComponent={ <ProFooter privacy={privacy} terms={terms} openSubscriptions={openSubscriptions}/> }
          style={{ backgroundColor: "white", marginTop: -3 }}
          numColumns={1}
          data={subscriptions}
          renderItem={(item) => (<ProMain item={item} handleBuySubscription={handleBuySubscription} owned={ownedSubscriptions} isPlay={isPlay} isIos={isIos}/>)}
        />
      </SafeAreaView>
     </SafeAreaProvider>
)
